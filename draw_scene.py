...
def draw_sketch(sketch: np.ndarray, save_path: str=None, is_absolute: bool=False, max_dim: int=224, white_bg: bool=False, margin: int=20):
    ...
    abs_sketch[:,0] -= xmin
    abs_sketch[:,1] -= ymin
    abs_sketch = normalize_to_scale_customized(abs_sketch, is_absolute=True, scale_factor=max_dim-(2*margin))
    abs_sketch[:,:2] += margin # pads 50 px to top and left sides
