...
# Author: Aleyna Kutuk
def coco_scene_graph_example_customized(image, objs, img_bboxes, sketch_bboxes, identifier, sketches, qd_class_ids, sample_ids, scene):
    ...
    feature = {
        ...
        'n_objs':  _int64_feature(len(objs)),
        'objs': _int64_list_feature(objs),
        'img_bboxes': _image_float_feature(img_bboxes),
        'sketch_bboxes': _image_float_feature(sketch_bboxes),
        'n_bboxes':  _int64_feature(sketch_bboxes.shape[0]),
        'id': _int64_feature(identifier),
        ...
        'qd_class_ids': _int64_list_feature(qd_class_ids.flatten()),
        'sample_ids': _int64_list_feature(sample_ids.flatten()),
        'scene': _image_float_feature(scene),
        'scene_size': _int64_feature(scene.shape[1]),
    }

    return tf.train.Example(features=tf.train.Features(feature=feature))


def parse_coco_scene_graph_record_customized(example_proto):
    image_feature_description = {
        'id': tf.io.FixedLenFeature([], tf.int64),
        'n_objs': tf.io.FixedLenFeature([], tf.int64),
        'size': tf.io.FixedLenFeature([], tf.int64),
        'image': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
        'scene': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
        'sketches': tf.io.FixedLenSequenceFeature([], tf.float32, allow_missing=True),
        'objs': tf.io.VarLenFeature(tf.int64),
        'img_bboxes': tf.io.VarLenFeature(tf.float32),
        'sketch_bboxes': tf.io.VarLenFeature(tf.float32),
        'n_bboxes': tf.io.FixedLenFeature([], tf.int64),
        'n_sketches': tf.io.FixedLenFeature([], tf.int64),
        'sketch_size': tf.io.FixedLenFeature([], tf.int64),
        'qd_class_ids': tf.io.VarLenFeature(tf.int64),
        'sample_ids': tf.io.VarLenFeature(tf.int64),
        'scene_size': tf.io.FixedLenFeature([], tf.int64),

    }
    parsed = tf.io.parse_single_example(example_proto, image_feature_description)
    image = tf.reshape(parsed['image'], [parsed['size'], parsed['size'], 3])
    img_bboxes = tf.reshape(parsed['img_bboxes'].values, [parsed['n_bboxes'], 4])
    sketch_bboxes = tf.reshape(parsed['sketch_bboxes'].values, [parsed['n_bboxes'], 4])
    sketches = tf.reshape(parsed['sketches'], [parsed['n_sketches'], parsed['sketch_size'], parsed['sketch_size'], 1])
    scene = tf.reshape(parsed['scene'], [parsed['scene_size'], parsed['scene_size'], 1])

    return (parsed['n_objs'], image, parsed['objs'].values, img_bboxes, sketch_bboxes, 
            parsed['n_bboxes'], parsed['id'], sketches, parsed['qd_class_ids'].values, parsed['sample_ids'].values, scene)
...
# Author: Aleyna Kutuk
def sketchycoco_scene_graph_example_customized(image, objs, boxes, identifier, sketches, scene):
    feature = {
        'image': _image_float_feature(image),
        'size': _int64_feature(image.shape[0]),
        'n_objs':  _int64_feature(len(objs)),
...
def parse_sketchycoco_scene_graph_record_customized(example_proto):
    image_feature_description = {
        'id': tf.io.FixedLenFeature([], tf.int64),
        'n_objs': tf.io.FixedLenFeature([], tf.int64),
        'size': tf.io.FixedLenFeature([], tf.int64),
    ...
    return parsed['n_objs'], image, parsed['objs'].values, boxes, parsed['id'], sketches, scene
