...
def load_all_data_and_save_in_chunks(image_ids, id_to_size, id_to_objects, 
    ...
    for i in tqdm(range(0, len(image_ids))):
        ...
        c_objs, c_boxes, c_boxes_orig = coco.preprocess(size, objs, hps, c_meta)
        ...
        if c_objs is not None:
            ...
            for i, coco_class in enumerate(coco_classes):
                ...
                raster_sketches[i] = draw_sketch(np.asarray(qd_data), save_dir, white_bg=True, max_dim=hps['obj_size'])
                sample_ids.append(sel_id)
                
                if hps["save_embeds"]:
                    sketch_temp = copy.deepcopy(np.asarray(qd_data))
...
def main():
    ...
    parser.add_argument('--target-dir', default='coco-records-latest')
