...
def draw_segmentation(
    sketch: np.ndarray, obj_division, class_ids, save_path: str=None, margin: int=10, keep_size: bool=False,
    ...
    for i in range(1, abs_sketch.shape[0]):
        if abs_sketch[i-1, -1] > 0.5:
            stroke_cnt += 1
            if stroke_cnt in obj_division:
                color_cnt += 1
            continue # stroke end
        
        fill_color = (class_ids[color_cnt], class_ids[color_cnt], class_ids[color_cnt])
